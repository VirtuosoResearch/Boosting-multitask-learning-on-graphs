...
def generate_sbm_graph(node_num, blocks=2, intraclass_prob=0.6, interclass_prob=0.1):
    ''' Generate a two block model graph '''
    
    sizes_list = [int(node_num/blocks)]*blocks 
    probs = np.eye(blocks)
    for i in range(blocks):
        for j in range(blocks):
            if i!=j:
                probs[i, j] = interclass_prob
                
    network = stochastic_block_model(
        sizes = sizes_list,  p = probs, seed=42,
    )
    ...
    np.random.seed(42)
    node_features = np.random.randn(node_num, 128)*10    
