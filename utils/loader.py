...
import numpy as np
from utils.sample import remove_redundant_edges, compute_effective_resistances

class OriginalGraphDataLoader(torch.utils.data.DataLoader):
...
class GraphSAINTEdgeSampler(GraphSAINTSampler):
    ...
    def __sample_nodes__(self, batch_size):
        ...
        rand = torch.rand(self.E).log() / (prob + 1e-10)
        edge_sample = rand.topk(batch_size).indices
...
class LeverageScoreEdgeSampler(GraphSAINTSampler):
    r"""The leverage score edge sampler class (see
    :class:`~torch_geometric.loader.GraphSAINTSampler`).
    """
    def __init__(self, data, batch_size: int, num_steps: int = 1, sample_coverage: int = 0, save_dir: Optional[str] = None, log: bool = True, 
                 Z_dir = None, **kwargs):
        ''' Obtain sampling probability '''
        edge_index = data.edge_index.clone()
        edge_index = remove_redundant_edges(edge_index)
        Z = np.load(Z_dir)
        probs = compute_effective_resistances(Z, edge_index.numpy())
        print("Successfully loaded leverage scores! Scores sum: {}".format(probs.sum()))
    ...
    def __sample_nodes__(self, batch_size):
        ...
        rand = torch.rand_like(self.probs).log() / (self.probs + 1e-10)
        edge_sample = rand.topk(batch_size).indices
